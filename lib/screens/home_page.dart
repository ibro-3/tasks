...
class TaskHomePage extends StatefulWidget {
  ...
  const TaskHomePage({super.key, required this.settings, required this.taskProvider});
...
class _TaskHomePageState extends State<TaskHomePage> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _confettiController.addListener(() { 
      setState(() {}); 
      if (_confettiController.status == AnimationStatus.completed) _particles.clear(); 
    });
  ...
  void _spawnConfetti() { 
    _particles.clear(); 
    for (int i = 0; i < 40; i++) { _particles.add(ConfettiParticle()); } 
      ...
      builder: (context, provider, _) {
        ...
        return Scaffold(
          ...
          drawer: CustomDrawer(
            ...
            onSettings: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SettingsPage(s: widget.settings))),
            onAddList: () => showDialog(
              context: context,
              builder: (ctx) => NewListDialog(
                existingNames: lists.map((l) => l.name).toList(),
                onCreate: (name, icon, color) {
                  provider.addList(TaskListModel(name: name, iconCode: icon, colorValue: color));
                  _switchList(name);
                }
              )
            ),
          ...
          body: Stack(fit: StackFit.expand, children: [
            _buildTaskList(
              context, theme, currentList, activeTasks, groupedTasks, progress, 
              isCustomList, provider, bottomPadding
            ),
            _buildBottomBar(theme, bottomPadding),
            if (_confettiController.isAnimating) 
              Positioned.fill(child: CustomPaint(painter: ConfettiPainter(_particles, _confettiController.value))),
          ]),
        );
      },
    );
  ...
  double _calculateProgress(List<Task> tasks) {
    final todayTasks = tasks.where((t) => 
      t.dueDate != null && t.dueDate!.day == DateTime.now().day && !t.isCompleted
    ).length;
    final totalToday = tasks.where((t) => 
      t.dueDate != null && t.dueDate!.day == DateTime.now().day
    ).length;
  ...
  Widget _buildTaskList(BuildContext context, ThemeData theme, String currentList, 
      List<Task> activeTasks, Map<String, List<Task>> groupedTasks, double progress, 
      bool isCustomList, TaskProvider provider, double bottomPadding) {
    ...
    return CustomScrollView(slivers: [
      SliverAppBar.large(
        ...
        title: _isSearchActive 
          ? Text("Results for '${_searchController.text}'", style: const TextStyle(fontSize: 22))
          : (currentList == 'My Day' 
              ? Row(children: [
        ...
        actions: [
          if (isCustomList && !_isSearchActive) 
            IconButton(
              ...
              onPressed: () => _confirmDeleteList(context, provider, currentList)
            )
        ],
      ),
      ...
      if (activeTasks.isEmpty) 
        SliverFillRemaining(
          child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.event_available_rounded, size: 80, color: theme.colorScheme.surfaceContainerHighest), 
            const SizedBox(height: 20), 
            Text(_isSearchActive ? "No matches" : "All clear", style: theme.textTheme.titleLarge?.copyWith(color: theme.colorScheme.secondary))
          ]))
        ),
        ...
        SliverPadding(
          ...
          sliver: SliverList(
            delegate: SliverChildBuilderDelegate((ctx, i) {
                  ...
                  if (currentList != 'My Day' || _isSearchActive) 
                    flatList.add(Padding(
                      ...
                      child: Text(key, style: TextStyle(
                        color: key == "Overdue" ? theme.colorScheme.error : theme.colorScheme.primary, 
              ...
              return null;
            }, childCount: activeTasks.length + ((currentList != 'My Day' || _isSearchActive) 
              ? groupedTasks.keys.where((k) => groupedTasks[k]!.isNotEmpty).length 
              : 0)))
        ),
    ]);
  }
  ...
  Widget _buildTaskItem(Task task, bool isOverdue, TaskProvider provider) {
    return Dismissible(
      ...
      child: TactileTaskCard(
        ...
        isOverdue: isOverdue, 
        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => TaskDetailScreen(
          task: task, 
          onUpdate: (nt) => provider.updateTask(nt), 
          onDelete: () => provider.updateTask(task, delete: true)
        ))), 
        onCheck: () { task.isCompleted = !task.isCompleted; _onTaskComplete(task, task.isCompleted); }, 
        onStar: () => provider.toggleStar(task)
      )
    );
  }

  void _confirmDeleteList(BuildContext context, TaskProvider provider, String listName) {
    showDialog(
      context: context,
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            child: _isSearchActive 
              ? Row(children: [
                  IconButton(padding: const EdgeInsets.all(12), icon: const Icon(Icons.arrow_back, size: 28), onPressed: _endSearch), 
                  Expanded(child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: _searchController.text.isNotEmpty 
                        ? IconButton(icon: const Icon(Icons.close, size: 20), onPressed: () => _searchController.clear()) 
                        : null
                    ), 
              ...
              : Row(children: [
                  ...
                  IconButton(padding: const EdgeInsets.all(12), icon: const Icon(Icons.menu, size: 28), onPressed: () => _scaffoldKey.currentState?.openDrawer()), 
                  const SizedBox(width: 4), 
                  IconButton(padding: const EdgeInsets.all(12), icon: const Icon(Icons.search, size: 28), onPressed: _startSearch), 
                  const Spacer(), 
                  Padding(
                    ...
                    child: Material(
                      ...
                      child: InkWell(
                        ...
                        child: Container(height: 56, width: 56, alignment: Alignment.center, child: Icon(Icons.add_rounded, color: theme.colorScheme.onPrimary, size: 32))
                      )
                    )
                  )
                ])
  ...
  void _showAddSheet(BuildContext ctx) {
    ...
    DateTime? d = widget.taskProvider.currentList == 'My Day' ? _selectedDate : null; 
    ...
    showModalBottomSheet(
      ...
      builder: (_) => StatefulBuilder(
        builder: (context, setSheetState) => Padding(
          padding: EdgeInsets.only(bottom: MediaQuery.of(ctx).viewInsets.bottom + 20, left: 20, right: 20), 
          child: Container(
            decoration: BoxDecoration(color: Theme.of(ctx).colorScheme.surface, borderRadius: BorderRadius.circular(widget.settings.radius)), 
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Row(children: [
                if (d != null) ActionChip(avatar: const Icon(Icons.calendar_today, size: 16), label: Text("${d!.month}/${d!.day}"), onPressed: () => setSheetState(() => d = null)) 
                else IconButton(icon: const Icon(Icons.calendar_today), onPressed: () async { final dp = await showDatePicker(context: ctx, initialDate: d ?? DateTime.now(), firstDate: DateTime.now(), lastDate: DateTime(2100)); if(dp!=null) setSheetState(()=> d = dp); }),
                ...
                const Spacer(),
                FloatingActionButton.small(onPressed: () { if (tc.text.isNotEmpty) { widget.taskProvider.addTask(tc.text, isStarred: isStarred, dueDate: d, notes: nc.text); Navigator.pop(ctx); } }, child: const Icon(Icons.arrow_upward))
              ])
            ])
          )
        )
